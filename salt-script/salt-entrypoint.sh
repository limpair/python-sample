...
start() {
    cd $WORK_DIR
    uwsgi --http-socket 0.0.0.0:8000 --wsgi-file $APP_NAME --callable app_dispatch --processes 1 --threads 2 --stats 0.0.0.0:9000 --uid nobody --gid nobody > /var/log/$APP_NAME.log 2>&1 &
}
stop() {
    kill -9 $(ps aux | grep python | grep sample | grep -v entrypoint | awk '{print $2}')
}
status() {
    RUN=`ps aux | grep python | grep sample | grep -v entrypoint | wc -l`
    if [ $RUN -gt 0 ]
    then
        echo 'is ok'
        exit 0
